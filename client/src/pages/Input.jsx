...
export default function Input({ onProfileData }) {
  ...
  const handleFetch = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/api/github/${username}`);
      ...
      {isLoading ? (
        ...
        <div className="bg-gray-800 p-6 rounded-md shadow-md w-full max-w-md">
          ...
          <p className="text-gray-400 mb-4">
            To fetch a GitHub profile, please enter the username in the format: <strong>https://github.com/username</strong>.
            We will retrieve the profile details and display them for you.
          </p>
          <input
            type="text"
            value={githubUrl}
          ...
          <button
            onClick={handleFetch}
            className="bg-blue-600 text-white p-3 rounded-md w-full hover:bg-blue-700 transition duration-300"
          >
            Fetch Profile
          </button>
